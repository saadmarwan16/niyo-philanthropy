...
const Blogs: NextPage<BlogsPageProps> = (props) => {
  ...
  const blog = useMemo(() => {
    if (blogs && blogs.data.length > 0) {
      return blogs.data[Math.floor(Math.random() * blogs.data.length)];
    }
            ...
            {blogs ? (
              <>
                {blogs.data.length > 0 ? (
                    ...
                    <div className="flex flex-col gap-4 py-16 sm:gap-6 md:gap-8 blog-campaign-horizontal-padding">
                      <div className="flex flex-col gap-2 sm:flex-row sm:justify-between sm:items-center">
                        ...
                        <span className="px-2 py-0.5 md:py-1 sm:px-3 md:px-4 border border-gray-500 rounded-lg text-sm sm:text-base w-fit">
                          {pagination?.startingCursor} -{" "}
                          {pagination?.endingCursor} of {pagination?.total}
                        </span>
                      </div>
                      ...
                      <div className="flex items-center justify-end">
                        <button
                          ...
                          onClick={() => {
                            setIsLoading(true);
                            blogsController
                              .getAll(blogs.meta.pagination.page - 1)
                              .then((res) => {
                  ...
                  <EmptyContent title="Blog" content="blogs" />
                )}
              </>
            ) : (
              <ErrorContent
